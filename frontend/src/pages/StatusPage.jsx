              ...
              <motion.div key={req.real_id || `match-${idx}`} initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: idx * 0.1 }} style={styles.card}>
                ...
                <div style={styles.body}>
                  ...
                  {req.group.map((peer, peerIdx) => (
                    <div key={peerIdx} style={{ background: 'white', padding: '15px', borderRadius: '10px', marginBottom: '10px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', borderLeft: `5px solid ${CT_RED}` }}>
        ...
        <div style={styles.column}>
          ...
          {queuedRequests.map((req, idx) => (
            <motion.div key={req.real_id || `queue-${idx}`} initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: idx * 0.1 }} style={styles.card}>
              <div style={{...styles.header, background: CT_RED}}>
                  <p style={{ margin: '0', fontSize: '1.1rem', fontWeight: 'bold' }}>{req.user.program}</p>
                  <p style={{ margin: '5px 0 0 0', opacity: 0.9 }}>{req.user.course}</p>
              </div>
...
const styles = {
  ...
  header: { background: colors.primary.berkeleyBlue, color: 'white', padding: '20px', textAlign: 'center', borderBottom: `4px solid #f32c2c` },
  body: { padding: '20px' },
  ...
  modalOkBtn: { padding: '12px 30px', background: '#f32c2c', color: 'white', border: 'none', borderRadius: '20px', fontWeight: 'bold', cursor: 'pointer' },
};
