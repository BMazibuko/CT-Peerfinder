...
const AdminPage = () => {
  ...
  const downloadCSV = async () => {
    try {
      ...
      triggerDownload(res.data, 'ct_peer_data.csv');
  ...
  const downloadFeedback = async () => {
    try {
      ...
      triggerDownload(res.data, 'ct_tool_feedback.csv');
  ...
  const downloadSessionFeedback = async () => {
    try {
      ...
      triggerDownload(res.data, 'ct_peer_session_feedback.csv');
  ...
  const downloadUnpairReasons = async () => {
    try {
      ...
      triggerDownload(res.data, 'ct_unpair_reason_data.csv');
    ...
    <div style={styles.dashboardContainer}>
        ...
        {activeTab === 'dashboard' && (
          <div>
            <div style={styles.statsGrid}>
              <StatCard title="Total Learners" value={totalInView} color={CT_RED} emoji="👥" />
            ...
            <div style={styles.chartsGrid}>
              <ChartBox title="By Short Course" data={courses} color={CT_RED} />
                  ...
                  <div style={styles.groupMembers}>
                    {members.map(m => (
                      <div key={m.id} style={styles.memberChip}>
                        ...
                        <span style={{fontSize:'0.8rem', color: CT_RED}}>
      ...
      {loading && (
        <div style={styles.modalOverlay}>
          <Spinner size="40px" color={CT_RED} />
        </div>
      )}
...
const styles = {
  ...
  activeTab: { padding: '10px 20px', background: 'white', borderBottom: `3px solid ${CT_RED}`, color: CT_RED, fontWeight: 'bold', cursor: 'pointer' },
  ...
  trSelected: { background: '#fce4e4', borderBottom: '1px solid #eee' }, // Light red tint for selected
  ...
  btnPrimary: { padding: '12px 20px', width:'100%', background: CT_RED, color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontWeight:'bold' },
  ...
  fab: { position: 'fixed', bottom: '30px', right: '30px', padding: '15px 25px', background: CT_RED, color: 'white', borderRadius: '30px', border: 'none', boxShadow: '0 4px 15px rgba(0,0,0,0.3)', fontWeight: 'bold', cursor: 'pointer', fontSize: '1.1rem', zIndex: 100 },
  ...
  btnConfirm: { padding: '8px 20px', background: CT_RED, color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontWeight:'bold' },
  ...
  pageBtn: { padding: '8px 16px', background: 'white', border: `1px solid ${CT_RED}`, color: CT_RED, borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold' },
