...
const PeerFeedbackPage = () => {
  ...
  // Creative Tech (CT) Programs Data
  const programs = {
    'GD': ['GD-1', 'GD-2', 'GD-3', 'GD-4', 'GD-5', 'GD-6'],
    'CC': ['CC-1', 'CC-2', 'CC-3', 'CC-4', 'CC-5', 'CC-6']
  };

  const CT_RED = "#f32c2c";

  // Full Form State preserved from original script
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Endpoint preserved from original script logic
      ...
      <div style={styles.container}>
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} style={styles.card}>
            ...
            <p style={{color: '#555', marginBottom: '30px', lineHeight: '1.6'}}>
              Thank you for keeping the Creative Tech PeerFinder running smoothly! <br/><br/>
            ...
            <button onClick={() => navigate('/')} style={{...styles.primaryBtn, background: CT_RED}}>Back to Home</button>
        </motion.div>
      </div>
    );
  }

  // --- LOGIC GATES (PRESERVED FROM ORIGINAL) ---
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <div style={styles.section}>
            ...
            <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginTop: '15px'}}>
                <div>
                    ...
                    {Object.keys(programs).map(p => <option key={p} value={p}>{p === 'GD' ? 'Graphic Design' : 'Content Creation'}</option>)}
                    </select>
                </div>
                <div>
                    ...
                    <option value="">--Select--</option>
                    {formData.program && programs[formData.program].map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                </div>
            </div>
            ...
            <AnimatePresence>
                {formData.role === 'HelpSeeker' && (
                    <motion.div initial={{opacity:0, height:0}} animate={{opacity:1, height:'auto'}} exit={{opacity:0, height:0}}>
                        <label style={{...styles.label, color: CT_RED}}>Your Volunteer's ALX Email Address *</label>
                        <p style={styles.hint}>Required so we can award them their well-deserved Legacy Points!</p>
                        <input style={{...styles.input, borderColor: CT_RED}} type="email" name="volunteer_email" value={formData.volunteer_email} onChange={handleChange} required placeholder="Volunteer's Email" />
                    </motion.div>
                )}
            </AnimatePresence>
          </div>
          ...
          {showRatings && (
            <motion.div initial={{opacity:0, height:0}} animate={{opacity:1, height:'auto'}}>
              <div style={styles.section}>
                <label style={styles.label}>Overall session rating? *</label>
                ...
                {formData.role !== 'Volunteer' && formData.overall_rating > 0 && (
                    <motion.div initial={{opacity:0}} animate={{opacity:1}} style={{marginTop: '15px'}}>
                        <label style={styles.label}>What best describes your progress now? *</label>
                ...
                <AnimatePresence>
                    {formData.overall_rating > 0 && formData.overall_rating <= 3 && (
                        <motion.div initial={{opacity:0, height:0}} animate={{opacity:1, height:'auto'}} exit={{opacity:0, height:0}} style={{marginTop: '15px'}}>
                            <label style={{...styles.label, color: CT_RED}}>Please tell us more: *</label>
                            <textarea style={{...styles.textarea, borderColor: CT_RED}} name="feedback_details" value={formData.feedback_details} onChange={handleChange} required placeholder="What worked well or could be improved?" />
                        </motion.div>
                    )}
                </AnimatePresence>
              </div>
          ...
          {/* --- 5. THE eHUB REDIRECT (SUPPORT) --- */}
          <AnimatePresence>
              {showEscalationLink && formData.role !== 'Volunteer' && (
                  <motion.div initial={{opacity:0, scale:0.95}} animate={{opacity:1, scale:1}} style={{background: '#f8f9fa', border: `1px solid ${CT_RED}`, padding: '15px', borderRadius: '8px', textAlign: 'center'}}>
                      ...
                      <p style={{fontSize: '0.9rem', color: '#555', marginBottom: '15px'}}>
                          PeerFinder is for peer-to-peer collaboration. For official escalated help, visit the ALX Circle.
                      </p>
                      <a href="https://ehub.alxafrica.com/community" target="_blank" rel="noopener noreferrer" style={{display: 'inline-block', background: colors.primary.berkeleyBlue, color: 'white', padding: '10px 20px', borderRadius: '20px', textDecoration: 'none', fontWeight: 'bold', fontSize: '0.9rem'}}>
                          Go to Circle Community &rarr;
                      </a>
                  </motion.div>
              )}
          </AnimatePresence>
          ...
          {(isFullGhost || isOtherNo || (showRatings && formData.overall_rating > 0 && (formData.role === 'Volunteer' || formData.progress))) && (
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" style={{...styles.primaryBtn, background: CT_RED}} disabled={loading}>
...
const styles = {
  container: { minHeight: '100vh', background: '#f4f7f6', padding: '40px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', fontFamily: fonts.main },
  backBtn: { alignSelf: 'flex-start', marginBottom: '20px', background: 'transparent', border: `1px solid #ddd`, color: '#666', padding: '8px 16px', borderRadius: '20px', cursor: 'pointer' },
  card: { background: 'white', padding: '2.5rem', borderRadius: '16px', width: '100%', maxWidth: '600px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)' },
  header: { textAlign: 'center', color: '#1a1a1a', marginBottom: '5px', fontSize: '1.8rem', fontWeight: 'bold' },
  subtext: { textAlign: 'center', color: '#666', marginBottom: '25px', fontSize: '0.95rem' },
  form: { display: 'flex', flexDirection: 'column', gap: '20px' },
  section: { background: '#fcfcfc', padding: '20px', borderRadius: '12px', border: '1px solid #eee' },
  label: { fontWeight: '600', fontSize: '0.95rem', color: '#333', marginBottom: '8px', display: 'block', marginTop: '10px' },
  ...
  stars: { display: 'flex', fontSize: '2.5rem', cursor: 'pointer', marginBottom: '15px', justifyContent: 'center' },
  star: { transition: 'color 0.2s' },
  primaryBtn: { width: '100%', padding: '15px', marginTop: '10px', border: 'none', borderRadius: '30px', color: 'white', fontWeight: 'bold', fontSize: '1.1rem', cursor: 'pointer' },
};
