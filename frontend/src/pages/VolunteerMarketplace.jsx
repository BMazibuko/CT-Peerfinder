...
const VolunteerMarketplace = () => {
  ...
  const handlePairClick = (volunteerId) => {
    // Navigate internally since we are on the CT app now
    navigate(`/register?program=${program}&course=${course}&connectionType=need&targetVolunteerId=${volunteerId}`);
  };

  const handleFallbackQueue = () => {
    // Navigate internally since we are on the CT app now
    navigate(`/register?program=${program}&course=${course}&connectionType=need`);
  };
  ...
  if (!program || !course) return null; // Safe fallback while redirecting
...
const styles = {
  ...
  logoText: { color: colors.primary.iris, fontWeight: '700', fontSize: '1rem' },
