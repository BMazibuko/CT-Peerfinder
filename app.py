...
def validate_registration(data):
    ...
    if data.get('program') not in ['CC', 'GD']: errors.append("Invalid program selected")
    ...
    for _, current_user in grp.iterrows():
        ...
        for _, peer in grp.iterrows():
            if peer['id'] != current_user['id']:
                ...
                display_name = peer['pseudonym'] if str(peer.get('pseudonym')) and peer['connection_type'] == 'offer' else peer['name']
        ...
        </div><br/>Best regards,<br/><strong>Creative Tech Peer Finder Team</strong>"""
...
@api_wrapper
def health(): return jsonify({"status": "active", "version": "CT_Modular_SmartVacuum"})
...
def register():
    ...
    if updated:
        ...
        Best regards,<br/><strong>Creative Tech Peer Finder Team</strong>"""
...
def status(identifier):
    df = download_csv()
    # Decode URL-encoded strings safely to handle cases like "%40" becoming "@"
    decoded_ident = urllib.parse.unquote(identifier).strip()
    ident_lower = decoded_ident.lower()
    
    # Robust match: matches true ID OR lowercased Email perfectly!
    user_requests = df[(df['id'] == decoded_ident) | (df['email'].str.lower() == ident_lower)].sort_values(by='timestamp', ascending=False)
        ...
        if emails:
            ...
            for g_email in emails:
                ...
                for idx, g_user in ghoster_rows.iterrows():
                    g_name = g_user.get('name', 'Learner')
                    g_prog = g_user.get('program', data.get('program', 'CC'))
                    g_type = g_user.get('connection_type', '')
